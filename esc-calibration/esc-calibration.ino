...
void loop()
{
  ...
  {                 // If motor 1, 2, 3 or 4 is selected by the user.
    ...
    {                                                         // When the throttle was in the lowest position do this.
      receiver_input_channel_3 = convert_receiver_channel(3); // Convert the actual receiver signals for throttle to the standard 1000 - 2000us.
      if (data == '1' || data == '5')
        esc_1 = receiver_input_channel_3; // If motor 1 is requested set the pulse for motor 1 equal to the throttle channel.
      else
        esc_1 = 1000; // If motor 1 is not requested set the pulse for the ESC to 1000us (off).
      if (data == '2' || data == '5')
        esc_2 = receiver_input_channel_3; // If motor 2 is requested set the pulse for motor 1 equal to the throttle channel.
      else
        esc_2 = 1000; // If motor 2 is not requested set the pulse for the ESC to 1000us (off).
      if (data == '3' || data == '5')
        esc_3 = receiver_input_channel_3; // If motor 3 is requested set the pulse for motor 1 equal to the throttle channel.
      else
        esc_3 = 1000; // If motor 3 is not requested set the pulse for the ESC to 1000us (off).
      if (data == '4' || data == '5')
        esc_4 = receiver_input_channel_3; // If motor 4 is requested set the pulse for motor 1 equal to the throttle channel.
      else
        esc_4 = 1000; // If motor 4 is not requested set the pulse for the ESC to 1000us (off).
...
void esc_pulse_output()
{
  ...
  while (PORTA >= 16)
  {                            // Execute the loop until digital port 26 to 29 is low.
    esc_loop_timer = micros(); // Check the current time.
    if (timer_channel_1 <= esc_loop_timer)
      PORTA &= B11101111; // When the delay time is expired, digital port 26 is set low.
    if (timer_channel_2 <= esc_loop_timer)
      PORTA &= B11011111; // When the delay time is expired, digital port 27 is set low.
    if (timer_channel_3 <= esc_loop_timer)
      PORTA &= B10111111; // When the delay time is expired, digital port 28 is set low.
    if (timer_channel_4 <= esc_loop_timer)
      PORTA &= B01111111; // When the delay time is expired, digital port 29 is set low.
  }
}
...
void read_gyro_data()
{
  ...
  if (eeprom_data[28] & 0b10000000)
    acc_x *= -1;                                  // Invert acc_x if the MSB of EEPROM bit 28 is set.
  acc_y = acc_axis[eeprom_data[29] & 0b00000011]; // Set acc_y to the correct axis that was stored in the EEPROM.
  if (eeprom_data[29] & 0b10000000)
    acc_y *= -1;                                  // Invert acc_y if the MSB of EEPROM bit 29 is set.
